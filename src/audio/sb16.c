...
bool sb16_reset(void) {
    // Write to reset port
    outb(SB16_RESET, 1);
    sleep_ms(3);  // Wait longer for reset
    outb(SB16_RESET, 0);
...
bool sb16_create_buffer(uint32_t buffer_id, uint32_t size) {
    if (buffer_id >= SB16_MAX_BUFFERS) return false;
...
void sb16_destroy_buffer(uint32_t buffer_id) {
    if (buffer_id >= SB16_MAX_BUFFERS) return;
...
bool sb16_start_playback(uint8_t *data, uint32_t size, bool loop) {
    if (!g_sb16_context.device.initialized) return false;
...
void sb16_stop_playback(void) {
    if (!g_sb16_context.device.playing) return;
