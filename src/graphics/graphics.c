...
bool graphics_set_mode(int width, int height, int bpp) {
    if (!g_graphics_initialized) return false;
...
void graphics_end_frame(void) {
    if (!g_graphics_initialized || !g_graphics_context.in_frame) return;
...
void graphics_wait_vsync(void) {
    if (!g_graphics_context.vsync_enabled) return;
...
void graphics_swap_buffers(void) {
    if (!g_graphics_initialized || !g_graphics_context.double_buffering_enabled) return;
    if (!g_graphics_context.front_buffer || !g_graphics_context.back_buffer) return;
...
bool graphics_lock_surface(GraphicsSurface *surface) {
    if (!surface) return false;
    if (surface->is_locked) return false; // Already locked
...
void graphics_set_pixel(GraphicsSurface *surface, int x, int y, Color color) {
    ...
    Point point = {x, y};
    if (!_graphics_is_point_visible(surface, point)) return;
    
    surface->pixels[y * surface->width + x] = color;
}

Color graphics_get_pixel(GraphicsSurface *surface, int x, int y) {
    if (!surface || !surface->pixels) return COLOR_BLACK;
    if (x < 0 || y < 0 || x >= surface->width || y >= surface->height) return COLOR_BLACK;
...
void graphics_blit_scaled(GraphicsSurface *dest, GraphicsSurface *src, Rectangle *src_rect, Rectangle *dest_rect) {
    if (!dest || !src || !dest->pixels || !src->pixels || !dest_rect) return;
...
void graphics_set_clip_rect(GraphicsSurface *surface, Rectangle *rect) {
    ...
    if (rect) {
        ...
        surface->clip_rect.width = rect->x + rect->width > surface->width ? 
                                   surface->width - surface->clip_rect.x : rect->width;
        surface->clip_rect.height = rect->y + rect->height > surface->height ? 
                                    surface->height - surface->clip_rect.y : rect->height;
    } else {
...
void graphics_draw_text_scaled(GraphicsSurface *surface, const char *text, Point position, Color color, int scale) {
    if (!surface || !text || scale < 1) return;
...
int graphics_text_width(const char *text, int scale) {
    ...
    while (*text) {
        if (*text == '\n') {
            if (current_line_width > width) width = current_line_width;
...
void graphics_get_stats(uint32_t *frames, uint32_t *dropped, uint32_t *render_time) {
    if (frames) *frames = g_graphics_context.frame_count;
    if (dropped) *dropped = g_graphics_context.dropped_frames;
    if (render_time) *render_time = g_graphics_context.render_time_us;
}
...
void Draw(int x, int y, int r, int g, int b) {
    ...
    GraphicsSurface *surface = g_graphics_context.double_buffering_enabled ? 
                               g_graphics_context.back_buffer : 
                               g_graphics_context.front_buffer;
...
void DrawMouse(int x, int y, int r, int g, int b) {
    ...
    static int mouse_pattern[] = {
        0b1111111111, 0b1111111110, 0b1111111100, 0b1111111000, 0b1111110000,
        0b1111100000, 0b1111000000, 0b1110000000, 0b1100000000, 0b1000000000,
    };
...
void DrawAtariString(char *str, int x, int y, int r, int g, int b, int scale) {
    if (!g_graphics_initialized || !str) return;
