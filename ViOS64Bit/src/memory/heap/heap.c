...
static int heap_validate_table(void *ptr, void *end, struct heap_table *table) {
  ...
  size_t total_blocks = table_size / VIOS_HEAP_BLOCK_SIZE;
...
static bool heap_validate_alignment(void *ptr) {
  return ((uintptr_t)ptr % VIOS_HEAP_BLOCK_SIZE) == 0;
}
...
uintptr_t heap_align_value_to_upper(uintptr_t val) {
  if ((val % VIOS_HEAP_BLOCK_SIZE) == 0) {
    return val;
  }

  val = (val - (val % VIOS_HEAP_BLOCK_SIZE));
  val += VIOS_HEAP_BLOCK_SIZE;
  return val;
}
...
bool heap_is_block_range_free(struct heap *heap, size_t starting_block,
                              size_t ending_block) {
  struct heap_table *table = heap->table;
...
void *heap_block_to_address(struct heap *heap, int64_t block) {
  return heap->saddr + (block * VIOS_HEAP_BLOCK_SIZE);
}

void heap_mark_blocks_taken(struct heap *heap, int64_t start_block,
  ...
  for (int64_t i = start_block; i <= end_block; i++) {
    ...
    if (heap->block_allocated_callback) {
      heap->block_allocated_callback(address, VIOS_HEAP_BLOCK_SIZE);
    }
  }
}
...
int64_t heap_address_to_block(struct heap *heap, void *address) {
  return ((int64_t)(address - heap->saddr)) / VIOS_HEAP_BLOCK_SIZE;
}

void *heap_malloc(struct heap *heap, size_t size) {
  size_t aligned_size = heap_align_value_to_upper(size);
  int64_t total_blocks = aligned_size / VIOS_HEAP_BLOCK_SIZE;
...
void *heap_realloc(struct heap *heap, void *old_ptr, size_t new_size) {
  ...
  size_t new_total_blocks = new_size_aligned / VIOS_HEAP_BLOCK_SIZE;
  size_t old_total_size = current_alloc_blocks * VIOS_HEAP_BLOCK_SIZE;
  ...
  if (current_alloc_blocks >= new_total_blocks) {
    ...
    if (new_total_blocks > 0) {
      heap->table->entries[starting_block + new_total_blocks - 1] &=
          ~HEAP_BLOCK_HAS_NEXT;
    }
  ...
  if (heap_is_block_range_free(heap, extension_start, extension_end)) {
    ...
    for (size_t i = extension_start; i < extension_end; i++) {
      heap->table->entries[i] =
          HEAP_BLOCK_TABLE_ENTRY_TAKEN | HEAP_BLOCK_HAS_NEXT;
    }
...
size_t heap_total_size(struct heap *heap) {
  return heap->table->total * VIOS_HEAP_BLOCK_SIZE;
}
