...
{
    return multiheap_allocation_block_count(multiheap, ptr) * VIOS_HEAP_BLOCK_SIZE;
}
...
void multiheap_free(struct multiheap* multiheap, void* ptr)
{
    ...
    if (paging_heap)
    {
        ...
        {
            void* virtual_address_for_block = (void*)((uintptr_t) ptr) + (i * VIOS_HEAP_BLOCK_SIZE);
...
void* multiheap_alloc_second_pass(struct multiheap* multiheap, size_t size)
{
    ...
    {
        void* block_addr = heap_zalloc(chosen_real_heap->heap, VIOS_HEAP_BLOCK_SIZE);
        if (!block_addr)
        {
