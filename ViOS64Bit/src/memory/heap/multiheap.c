...
struct multiheap *multiheap_new(struct heap *starting_heap) {
  struct multiheap *multiheap =
      heap_zalloc(starting_heap, sizeof(struct multiheap));
  if (!multiheap) {
    goto out;
  }
...
multiheap_get_last_heap(struct multiheap *multiheap) {
...
struct multiheap_single_heap *
multiheap_get_heap_for_address(struct multiheap *multiheap, void *address) {
  struct multiheap_single_heap *current = multiheap->first_multiheap;
  while (current) {
...
void *multiheap_virtual_address_to_physical(struct multiheap *multiheap,
                                            void *ptr) {
  void *phys_addr =
      (void *)((uintptr_t)ptr - ((uintptr_t)multiheap->max_end_data_addr));
  return phys_addr;
}

struct multiheap_single_heap *
multiheap_get_paging_heap_for_address(struct multiheap *multiheap,
                                      void *address) {
  struct multiheap_single_heap *current = multiheap->first_multiheap;
  while (current) {
...
void multiheap_get_heap_and_paging_heap_for_address(
    struct multiheap *multiheap, void *ptr,
    struct multiheap_single_heap **heap_out,
    struct multiheap_single_heap **paging_heap_out, void **real_phys_addr) {
  void *real_addr = ptr;
...
void *multiheap_realloc(struct multiheap *multiheap, void *old_ptr,
                        size_t new_size) {
  ...
  multiheap_get_heap_and_paging_heap_for_address(multiheap, old_ptr, &phys_heap,
                                                 &paging_heap, &real_phys_addr);

  if (paging_heap) {
    panic("Reallocation not yet supported for virtual addresses whose address "
...
size_t multiheap_allocation_block_count(struct multiheap *multiheap,
                                        void *ptr) {
  struct multiheap_single_heap *paging_heap = NULL;
  struct multiheap_single_heap *phys_heap = NULL;
  struct multiheap_single_heap *heap_to_check = NULL;
  void *real_phys_addr = NULL;
  multiheap_get_heap_and_paging_heap_for_address(multiheap, ptr, &phys_heap,
                                                 &paging_heap, &real_phys_addr);
...
size_t multiheap_allocation_byte_count(struct multiheap *multiheap, void *ptr) {
  return multiheap_allocation_block_count(multiheap, ptr) *
         VIOS_HEAP_BLOCK_SIZE;
}

int multiheap_add_heap(struct multiheap *multiheap, struct heap *heap,
                       int flags) {
  ...
  struct multiheap_single_heap *new_heap = heap_zalloc(
      multiheap->starting_heap, sizeof(struct multiheap_single_heap));
  if (!new_heap) {
    return -ENOMEM;
  }
...
int multiheap_add_existing_heap(struct multiheap *multiheap, struct heap *heap,
                                int flags) {
  flags |= MULTIHEAP_HEAP_FLAG_EXTERNALLY_OWNED;
  return multiheap_add_heap(multiheap, heap, flags);
}

int multiheap_add(struct multiheap *multiheap, void *saddr, void *eaddr,
                  int flags) {
  struct heap *heap =
      heap_zalloc(multiheap->starting_heap, sizeof(struct heap));
  struct heap_table *table =
      heap_zalloc(multiheap->starting_heap, sizeof(struct heap_table));
  if (!heap || !table) {
    return -ENOMEM;
  }
...
void multiheap_free(struct multiheap *multiheap, void *ptr) {
  ...
  if (paging_heap) {
    size_t total_blocks =
        heap_allocation_block_count(paging_heap->paging_heap, ptr);
    size_t starting_block =
        heap_address_to_block(paging_heap->paging_heap, ptr);
    size_t ending_block = starting_block + total_blocks;
    for (size_t i = starting_block; i < ending_block; i++) {
      void *virtual_address_for_block =
          (void *)((uintptr_t)ptr) + (i * VIOS_HEAP_BLOCK_SIZE);
      void *data_phys_addr = paging_get_physical_address(
          paging_current_descriptor(), virtual_address_for_block);
...
void *multiheap_alloc_paging(struct multiheap *multiheap, size_t size,
                             struct multiheap_single_heap **eligible_heap_out) {
  void *allocation_ptr = NULL;
  size_t total_required_blocks = size / VIOS_HEAP_BLOCK_SIZE;
  struct multiheap_single_heap *current = multiheap->first_multiheap;
  while (current != 0) {
    if (!multiheap_heap_allows_paging(current)) {
      current = current->next;
...
void *multiheap_alloc_second_pass(struct multiheap *multiheap, size_t size) {
  ...
  size_t total_blocks = size / VIOS_HEAP_BLOCK_SIZE;
  struct multiheap_single_heap *chosen_real_heap = NULL;

  void *defragmented_virtual_memory_saddr =
      multiheap_alloc_paging(multiheap, size, &chosen_real_heap);
  if (!defragmented_virtual_memory_saddr) {
    allocation_ptr = NULL;
    goto out;
  }

  void *defragmented_virtual_memory_current_addr =
      defragmented_virtual_memory_saddr;
  allocation_ptr = defragmented_virtual_memory_saddr;

  for (size_t i = 0; i < total_blocks; i++) {
    void *block_addr =
        heap_zalloc(chosen_real_heap->heap, VIOS_HEAP_BLOCK_SIZE);
    if (!block_addr) {
      panic("Something went wrong, is there not enough bytes in physical heap "
            "but there is in paging heap, this mus ta bug");
    }

    paging_map(paging_desc, defragmented_virtual_memory_current_addr,
               block_addr, PAGING_IS_WRITEABLE | PAGING_IS_PRESENT);
    defragmented_virtual_memory_current_addr += (uint64_t)VIOS_HEAP_BLOCK_SIZE;
  }
...
int multiheap_ready(struct multiheap *multiheap) {
  ...
  while (current) {
    if (multiheap_heap_allows_paging(current)) {
      void *paging_heap_starting_address =
          max_end_addr + (uint64_t)current->heap->saddr;
      void *paging_heap_ending_address =
          max_end_addr + (uint64_t)current->heap->eaddr;

      struct heap_table *paging_heap_table =
          heap_zalloc(multiheap->starting_heap, sizeof(struct heap_table));
      paging_heap_table->entries = heap_zalloc(
          multiheap->starting_heap,
          current->heap->table->total * sizeof(HEAP_BLOCK_TABLE_ENTRY));
      paging_heap_table->total = current->heap->table->total;

      struct heap *paging_heap =
          heap_zalloc(multiheap->starting_heap, sizeof(struct heap));
      heap_create(paging_heap, paging_heap_starting_address,
                  paging_heap_ending_address, paging_heap_table);

      paging_map_to(paging_current_descriptor(), paging_heap_starting_address,
                    paging_heap_starting_address, paging_heap_ending_address,
                    0);
