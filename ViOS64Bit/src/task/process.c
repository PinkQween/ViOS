...
{
    int res = 0;
    paging_map_to(process->paging_desc, (void *)VIOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
int process_map_memory(struct process *process)
{
    ...
    paging_map_to(process->paging_desc, (void *)VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack + VIOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
out:
    return res;
}
...
{
    return (uintptr_t)addr >= VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END &&
           (uintptr_t)addr <= VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
}
...
int process_get_allocation_by_addr(struct process *process, void *addr, struct process_allocation_request *allocation_request_out)
{
    ...
    if (process_is_stack_memory(process, addr))
    {
        ...
        uint64_t stack_size = VIOS_USER_PROGRAM_STACK_SIZE;
        // START OF THE STACK IS HIGHER IN MEMORY REMEMBER
        uint64_t total_bytes_left = VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START - addr_int;
        allocation_request_out->allocation.ptr = (void *)VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END;
        allocation_request_out->allocation.end = (void *)VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
        ...
        allocation_request_out->peek.end = (void *)VIOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
