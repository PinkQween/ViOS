...
terminal_abs_x_for_next_character(struct terminal *terminal) {
  return terminal->bounds.abs_x +
         (terminal->text.col * terminal->font->bits_width_per_character);
}

inline static size_t
terminal_abs_y_for_next_character(struct terminal *terminal) {
...
struct terminal *terminal_create(struct graphics_info *graphics_info,
                                 int starting_x, int starting_y, size_t width,
                                 size_t height, struct font *font,
...
struct terminal *
terminal_get_at_screen_position(size_t x, size_t y,
  ...
  for (size_t i = 0; i < total_children; i++) {
    ...
    if (x >= terminal->bounds.abs_x && x <= terminal->bounds.width &&
        y >= terminal->bounds.abs_y && x <= terminal->bounds.height) {
...
void terminal_background_save(struct terminal *terminal) {
  ...
  memcpy(terminal->terminal_background, terminal->graphics_info->pixels,
         buffer_size);
}
...
static void terminal_update_positon_after_draw(struct terminal *terminal) {
  terminal->text.col += 1;
...
static bool terminal_bounds_check(struct terminal *terminal, size_t abs_x,
                                  size_t abs_y) {
  ...
  return (abs_x >= starting_x && abs_x <= ending_x && abs_y >= starting_y &&
          abs_y <= ending_y);
}

void terminal_restore_background(struct terminal *terminal, int sx, int sy,
  ...
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      ...
      background_pixel =
          terminal->terminal_background[y * terminal->bounds.width + x];
      graphics_draw_pixel(terminal->graphics_info, abs_x, abs_y,
                          background_pixel);
    }
  }
  ...
  graphics_redraw_graphics_to_screen(terminal->graphics_info, abs_x, abs_y,
                                     width, height);
}
...
int terminal_backspace(struct terminal *terminal) {
  ...
  terminal_cursor_set(terminal, current_col, current_col);
...
int terminal_write(struct terminal *terminal, int c) {
  ...
  font_draw(terminal->graphics_info, terminal->font, abs_x, abs_y, c,
            terminal->font_color);
  terminal_update_positon_after_draw(terminal);
...
int terminal_pixel_set(struct terminal *terminal, size_t x, size_t y,
  ...
  size_t abs_x = terminal->bounds.abs_x;
  size_t abs_y = terminal->bounds.abs_y;
...
void terminal_transparancy_key_set(struct terminal *terminal,
                                   struct framebuffer_pixel pixel_color) {
  graphics_transparency_key_set(terminal->graphics_info, pixel_color);
}

void terminal_transparency_ket_remove(struct terminal *terminal) {
...
int terminal_draw_rect(struct terminal *terminal, uint32_t x, uint32_t y,
  ...
  graphics_draw_rect(terminal->graphics_info, abs_x, abs_y, width, height,
                     pixel_color);
out:
  return res;
}
