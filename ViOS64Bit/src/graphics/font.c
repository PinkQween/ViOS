...
int font_draw_from_index(struct graphics_info *graphics_info, struct font *font,
                         int screen_x, int screen_y, int index_character,
                         struct framebuffer_pixel font_color);
struct font *font_load_from_image(const char *filename, size_t pixel_width,
                                  size_t pixel_height,
                                  size_t y_offset_per_character);

struct font *font_get_system_font() { return system_font; }
...
struct font *font_load_from_image(const char *filename, size_t pixel_width,
  ...
  size_t total_required_bytes_per_character =
      total_required_bits_per_character / 8;
    ...
    for (int col = 0; col < characters_per_row; col++) {
      ...
      for (size_t x = 0; x < pixel_width; x++) {
        for (size_t y = y_offset_per_character; y < pixel_height; y++) {
          ...
          image_pixel_data pixel =
              graphics_image_get_pixel(img_font, abs_x, abs_y);
          if (pixel.R != 0 || pixel.B != 0 || pixel.G != 0) {
            // if its not black then its a valid pixel for the character bitmap
            size_t char_offset =
                character_index * total_required_bytes_per_character;
  ...
  return font_create(character_data, total_characters, pixel_width,
...
struct font *font_load(const char *filename) {
  ...
  loaded_font = font_load_from_image(
      filename, FONT_IMAGE_CHARACTER_WIDTH_PIXEL_SIZE,
      FONT_IMAGE_CHRACTER_HEIGHT_PIXEL_SIZE, FONT_IMAGE_CHARACTER_Y_OFFSET);
  if (loaded_font) {
...
struct font *font_create(uint8_t *character_data, size_t character_count,
  ...
  font->bits_height_per_character = bits_height_per_chracter;
  font->subtract_from_ascii_char_index_for_drawing =
      subtract_from_ascii_char_index_for_drawing;
  return font;
}
...
int font_draw_from_index(struct graphics_info *graphics_info, struct font *font,
  ...
  size_t total_required_bits_per_character =
      font->bits_width_per_character * font->bits_height_per_character;
...
int font_draw(struct graphics_info *graphics_info, struct font *font,
              int screen_x, int screen_y, int character,
              struct framebuffer_pixel font_color) {
...
int font_system_init() {
  ...
  if (!system_font) {
    // Don't panic - just return an error and let the system continue without
    // fonts This allows us to see if other graphics features work
    res = -EIO;
    goto out;
  }
